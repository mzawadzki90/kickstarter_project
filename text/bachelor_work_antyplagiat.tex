...
% Title page [EN]
    ...
    \begin{itemize}
        ...
        \item \textit{natural selection} - Concept derived from Charles Darwin's theory of evolution, playing a key role in it; determines which individuals in the population will have a better chance to reproduce; it favors individuals that compete for available resources most efficiently\cite{IntroductionToEvolutionaryComputing2015}.
        ...
        \item \textit{mutation} - A stochastic operation that introduces a random change to an individual's genotype, which results in creation of a mutant\cite{IntroductionToEvolutionaryComputing2015}.
        \item \textit{artificial neural network}, ANN - A computing system designed to process information, inspired by the biological nervous system\cite{LeksykonSieciNeuronowych2015}.
    ...
    Sekcja trzecia jest wprowadzeniem do tematu algorytmów genetycznych. Opisano w niej, w telegraficznym skrócie odkrycia Charlesa Darwina i Gregora Johanna Mendla, które następnie posłużyły Alanowi Turingowi jako baza dla jego `ewolucyjnego poszukiwania' - czyli pierwowzoru wspomnianego algorytmu. W dalszej części omówiono cykl życia populacji w ramach algorytmu. Potem kolejno opisano jego kluczowe komponenty: inicjację populacji, ewaluację osobników, selekcję rodziców, krzyżowanie, mutację genomu, wybór potomstwa i warunek końcowy.

    \bigskip

    Sekcja czwarta to opis wybranego zbioru danych. Po przedstawieniu najważniejszych informacji opisano szczegółowo wszystkie cechy, wykorzystując do tego celu dokumentację deweloperską udostępnioną przez Spotify. Dodatkowo dla wszystkich parametrów dyskretnych utworzono czytelne histogramy, a dla flag i kategorii - wykresy kołowe.

    \bigskip

    W sekcji piątej opisano przygotowania do przeprowadzenia eksperymentu w tym wstępną obróbkę danych, bazową architekturę sieci neuronowej (SN) wykorzystaną w algorytmie, wybrane elementy SN, selekcję hiperparametrów do wyznaczenia oraz najważniejsze komponenty implementacji algorytmu genetycznego.

    \bigskip

    W sekcji szóstej zaprezentowano przebieg wykonania algorytmu genetycznego: zmiany wartości funkcji dopasowania dla najgorszego i najlepszego osobnika, średnią oraz medianę. Pokazano również, jak zmieniały się wartości poszczególnych hiperparametrów dla najlepszego osobnika. Następnie przedstawiono wyniki dokładnego uczenia finalnego modelu. Wyniki te zestawiono z rezultatami uzyskanymi po wytrenowaniu innych modeli o podobnej architekturze. Na koniec pokazano przykłady najlepszych i najgorszych predykcji finalnego modelu.

    \bigskip

    W ostatniej sekcji podsumowano prace. Zaproponowano modyfikacje, które mogłyby przyczynić się do poprawy wyników. Oceniono wyniki. Wypunktowano również czynniki, które przypuszczalnie miały największy wpływ na wielkość błędów.

    \subsection{Abstract}

    The first section describes in details the components shaping musical preferences. The following section covers the meaning and operating principle of music platforms, with an emphasis on Spotify from Sweden. This is also where the dataset, that is the subject of this study's research, comes from. Next, the most famous experiments with estimating the popularity of songs based on their musical characteristics are mentioned. In the end, the chances of creating a high-class model are assessed, taking into account the available data and tools.

    \bigskip

    The second section briefly describes the history of the creation of artificial neural networks and the biological inspirations behind it. Then, the structure and operating principle of the perceptron, one of the first implementations of neural networks, is discussed. Finally, the principle of the backpropagation algorithm is presented - the basic method of teaching deep neural networks.

    \bigskip

    The third section introduces genetic algorithms. It briefly describes the discoveries of Charles Darwin and Gregor Johann Mendel, which then served Alan Turing as the basis for his `evolutionary search' - that is the prototype of the above-mentioned algorithm. The following section discusses the life cycle of the population as part of the algorithm. The key components are then described: population initiation, individual evaluation, parent selection, crossover, genome mutation, offspring selection and the end condition.

    \bigskip

    The fourth section is a description of the selected dataset. After presenting the highlights, all features are detailed using information from the development documentation provided by Spotify. Additionally, clear histograms are created for all discrete parameters, and pie charts for flags and categories.

    \bigskip

    The fifth section describes the preparations for the experiment, including the initial data processing, the basic artificial neural network (ANN) architecture used in the algorithm, specific ANN elements, the selection of hyperparameters to be determined, and the most important components of the implementation of the genetic algorithm.

    \bigskip

    The sixth section presents the execution phase of the genetic algorithm: changes in the value of the fitness function for the worst and best individual, the mean, and the average. It also shows how the values of individual hyperparameters changed for the best individual. Then the results of the comprehensive training of the final model are presented. These results are compared with those obtained after training other models with similar architecture. Finally, examples of the best and worst predictions of the final model are shown.

    \bigskip

    The work is summarized in the last section. Modifications are proposed that could improve the results. The results are assessed. Factors that probably had the greatest impact on the size of errors are also listed.
    ...
    Wejście na rynek platform streaming-owych, których sztandarowym przykładem jest szwedzki gigant Spotify, dokonało małej rewolucji na rynku muzycznym. Platforma działa w dwóch modelach płatności: bezpłatnym (z reklamami) i subskrypcyjnym. Można korzystać z niej za pośrednictwem przeglądarki, ale dostępne są także dedykowane aplikacje na praktycznie każdy system, zaczynając od smartcaru na smartfonie kończąc, między którymi można się swobodnie przełączać. Muzyki można słuchać zarówno w trybie online, jak i w offline (oczywiście po wcześniejszym pobraniu na urządzenie). Co ważne zmianie uległ sposób konsumpcji samych treści. Autor niniejszej pracy, jako szczególnie aktywny użytkownik platformy, uznał za stosowne wymienienie następujących alternatywnych kanałów:
    \begin{itemize}
    ...
    Model neuronu zaproponowany przez McCullocha i Pittsa, znany jako `sztuczny neuron' (ang. \textit{artificial neuron}), ma conajmniej jedno wejście i jedno wyjście binarne (stan 0 lub 1). Wyjście aktywuje się wyłącznie przy określonej kombinacji sygnałów na wejściu. Udowodniono, że jest to wystarczające do stworzenia sieci rozwiązującej dowolne zadanie logiczne\cite{UczenieMaszynowe2018}. Na rysunku~\ref{fig:simple_logic_operations_on_artificial_neurons} przedstawiono realizację prostych operacji logicznych z wykorzystaniem sztucznych neuronów.
    ...
    Łączy ona operację odwrotnego różniczkowania i metodę gradientu prostego.
    ...
    Algorytm genetyczny jest z założenia generyczny - trzeba go traktować jako szablon, w którym, w zależności od potrzeb można wymieniać komponenty składowe. Te komponenty to:
    \begin{itemize}
    ...
    Warunek końcowy, kończy cykl życia populacji i tym samym cały algorytm. Wyróżnia się dwa główne przypadki\cite{IntroductionToEvolutionaryComputing2015}:
    \begin{itemize}
    ...
    Zbiór danych zawiera informacje o ponad 170 tysiącach utworów muzycznych, opublikowanych w latach 1921--2020 i dostępnych na szwedzkiej platformie streamingowej Spotify. Dataset został stworzony przy wykorzystaniu oficjalnego, publicznego API deweloperskiego Spotify i udostępniony na platformie Kaggle w formacie CSV przez użytkownika Yamaç Eren Ay\cite{SpotifyKaggleDataset2020}.
    ...
    \begin{itemize}
        ...
        \item \textit{energy} (pl. energia) - Miara w skali od 0.0 do 1.0 reprezentująca odczuwalny stopień intensywności i aktywności utworu. Typowe utwory `energetyczne' są odbierane jako szybkie, głośne czy wręcz hałaśliwe. Przykładem wysokiej energetyczności mogą byś utwory death-metalowe, podczas gdy preludia Bacha będą cechowały się niską energetycznością. Postrzegalne czynniki wpływające na tę cechę to: dynamiczny zakres, odbierana głośność, tembr, \textit{onset rate} (pl. współczynnik rozpoczęć ??) i ogólna entropia.
        ...
        \item \textit{instrumentalness} (pl. instrumentalność) - Miara prawdopodobieństwa określająca, czy dany utwór nie zawiera wstawek wokalnych (fragmentów śpiewanych). Wyrazy dźwiękonaśladowcze takie jak `ooh' czy `aah' przez algorytm/klasyfikator są traktowane jako instrumentalne. Rap czy zwykła mowa traktowana jako `czysty wokal'. Im bliżej wartości 1.0, tym wyższe prawdopodobieństwo braku wokali. Wartości powyżej 0.5 w założeniu mają reprezentować utwory instrumentalne, natomiast prawdopodobieństwo prawidłowej klasyfikacji rośnie wraz ze zbliżaniem się wskaźnika do wartości 1.0.
    ...
    \end{itemize}
    \smallskip
    Opisy parametrów są luźnym tłumaczeniem oficjalnej dokumentacji API deweloperskiego Spotify\cite{SpotifyWebAPIReference}. \textbf{Metody wyznaczania wartości metryk subiektywnych są własnością intelektualną firmy Spotify i nie są dostępne publicznie.}
    ...
    \subsection{Selekcja i wstępna obróbka danych}
    ...
    Podczas badań nad procesem uczenia głębokich sieci neuronowych naukowcy natrafili na tak zwany `problem zanikających gradientów' (malenie wartości gradientów funkcji kosztu wraz z propagacją wsteczną na coraz niższe warstwy sieci) oraz będący jego przeciwieństwem `problem wybuchających gradientów' (stały wzrost gradientów powodujący rozbieżność algorytmu (problem dotyczący głównie rekurencyjnych sieci neuronowych)\cite{UczenieMaszynowe2018}. Była to przeszkoda na tyle poważna, że na długie lata wstrzymała rozwój prac nad głębokimi sieciami neuronowymi. Dopiero w 2010 roku Xavierowi Glorot i Yoshue Benglo udało się zdiagnozować przyczynę problemu i zaproponować skuteczne rozwiązanie. Przyczyną okazało się powszechne wykorzystywanie kombinacji logistycznej i sigmoidalnej funkcji aktywacji, oraz inicjacji wag losowymi wartościami o rozkładzie normalnym i średniej wynoszącej 0, oraz odchyleniu standardowym równym 1. Badacze doszli do wniosku, że prawidłowy przepływ informacji między warstwami sieci jest możliwy jedynie, gdy wariancja dla wyjść warstwy jest równa wariancji wejść tejże, a dodatkowo gradienty funkcji kosztu muszą mieć taką samą wariancję przed i po przejściu przez daną warstwę w procesie propagacji wstecznej\cite{UnderstandingTheDifficultyOfTrainingDeepFeedforwardNeuralNetworks2010}.
    ...
    \begin{itemize}
        \item $monitor = val\_loss$ - monitorowana metryka to strata na zbiorze walidacyjnym (MSE walidacyjne)
    ...
    Dodatkowo do mechanizmu uczącego podpięto Keras CSVLogger - mechanizm logujący, zbierający wartości metryk ze wszystkich epok i zapisujący je do pliku CSV, a także customowy logger stworzony przez autora i podpięty bezpośrednio pod kod dla algorytmu genetycznego, który zapisywał do pliku CSV następujące dane: numer pokolenia, wartość funkcji dopasowania (FF) dla najlepszego osobnika w populacji, wartości poszczególnych parametrów dla najlepszego osobnika, FF dla najgorszego osobnika, wartości poszczególnych parametrów dla najgorszego osobnika, średnia FF, mediana FF, odchylenie standardowe FF\@.

    \bigskip

    Poniżej przedstawiono wykresy ilustrujące przebieg algorytmu genetycznego:
    ...
    Poniżej przedstawiono wykresy zmiany wartości parametrów dla najlepszego osobnika:
    ...
    \begin{figure}[H]
        ...
        \caption{Wykres zmiany wartości współczynnika porzucenia w warstwie ukrytej 1 dla najlepszego osobnika w populacji.}
    ...
    \begin{figure}[H]
        ...
        \caption{Wykres zmiany wartości współczynnika porzucenia w warstwie ukrytej 2 dla najlepszego osobnika w populacji.}
    ...
    \begin{figure}[H]
        ...
        \caption{Wykres zmiany wartości współczynnika porzucenia w warstwie ukrytej 3 dla najlepszego osobnika w populacji.}
    ...
    \begin{figure}[H]
        \centering
        \frame{\includegraphics[keepaspectratio, width=1.5\textwidth, angle=90]{tensorboard_graph}}
    ...
    Poniżej przedstawiono wykresy obrazujące przebieg procesu uczenia modelu stworzonego w oparciu o hiperparametry wyznaczone przez algorytm genetyczny:

    \begin{figure}[H]
        \centering
        \includegraphics[width=\textwidth]{f_m_mse_train_valid}
        \caption{Wykres zestawiający wartości metryk MSE dla zbioru treningowego i walidacyjnego podczas procesu uczenia sieci neuronowej.}
    ...
    \begin{figure}[H]
        ...
        \caption{Wykres zestawiający wartości metryk MAE dla zbioru treningowego i walidacyjnego podczas procesu uczenia sieci neuronowej.}
    ...
    \begin{figure}[H]
        ...
        \caption{Wykres zestawiający wartości metryk RMSE dla zbioru treningowego i walidacyjnego podczas procesu uczenia sieci neuronowej.}
        \label{fig:f_m_rmse_train_valid}
    \end{figure}

    Otrzymane wyniki zestawiono z rezultatami otrzymanymi dla mniejszych modeli zaprojektowanych w oparciu o metodę `prób i błędów' i wskazówki z literatury przedmiotu, a także z siecią 2-warstwową zaprojektowaną z wykorzystaniem algorytmu genetycznego. Porównanie przykładowych architektur sieci neuronowych z wybranym modelem:
